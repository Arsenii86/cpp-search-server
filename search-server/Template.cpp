...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentStatus status_req= DocumentStatus::ACTUAL) const{
    ...
    vector<Document> FindAllDocuments(const Query& query, KeyMapper key_mapper) const {
        ...
        for (const string& word : query.plus_words) {
            ...
            for (const auto [document_id, term_freq] : word_to_document_freqs_.at(word)) {
                ...
                auto LAmbdaFunction= [key_mapper](int document_id, DocumentStatus status, int rating){return key_mapper(document_id,status,rating);}; 
                if (LAmbdaFunction(document_id,status,rating)) {
